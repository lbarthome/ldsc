#!/usr/bin/env python3
"""
Reads ldsc/Builds/baseline.*.annot.gz files for the chromosome given in argument
...
def get_esnps(chr, INPUT_PATH_EQTL):
    """
    Takes a chromosome chr (just an integer) and returns the corresponding list 
    of  eSNPs in a panda datafrane contianing one column of rsID (called SNP) 
    and a column of logical value for beeing a siginficant eQTL (all here are !)
    """
    INPUT_COLUMNS = [
        "rsID" #, "ref", "alt"
    ]    
...
def get_baseline_annot(chr, INPUT_PATH_BASELINE):
    """
    Takes a chromosome chr (just an integer) and returns the corresponding list
    of SNPs annotated in ldsc baseline model in a panda datafrane containing 
    columns to identify the SNP (chr, bp, snp, cm) and then one column per
    category with logical values (0 or 1) depending on the SNP appartenance
...
def build_new_annot(esnps, baseline_annot):
    ...
    a annotation table with a new column corresponding to eSNPs. An eSNP
    significant in GTEx will have a 1, others a 0 (not using a list of tested
    SNPs in GTEx
    """
    new_annot = pd.merge(
...
if __name__ ==  "__main__":
    ...
    if len(args) <1:
        print(
            "Error : missing argument. Try again indicating the chr as an int"
        )
        sys.exit(1)
